...
1. [Lifecycle configuration](#lifecycle-configuration)
   ...
   1. [Incomplete multipart uploads](#incomplete-multipart-uploads)
   1. [Rule precedence](#rule-precedence)
...
> [!warning]
> Do **not** enable these opt-in tiers on buckets used by applications that require real-time access (e.g. Grafana
> Mimir, Grafana Loki, OpenSearch). The default Intelligent-Tiering tiers (Frequent, Infrequent, Archive Instant Access)
> are all real-time readable with millisecond latency, but Archive Access and Deep Archive Access behave like Glacier,
> with retrieval times of minutes to hours. This will break queries for data that has been archived.
...
S3 supports lifecycle configurations to automate object management.<br/>
[_Transition_ actions][transitions] **move** objects to a different storage class after a specified time.<br/>
[_Expiration_ actions][expiration] **delete** objects (or object versions) after a specified time.<br/>
_AbortIncompleteMultipartUpload_ cleans up incomplete multipart upload parts after a specified number of days.

Rules can be scoped using prefix filters, tag filters, or object size filters.<br/>
Tag-based rules are evaluated **daily**. When an object matches, the action is queued and the object's tags are checked
again during execution; if the triggering tag was removed in between, the action does not proceed. Removing a tag does
**not** guarantee an already queued action is cancelled.

Rule updates can take up to 15 minutes to propagate. Prefer removing the tag over disabling the lifecycle rule when
wanting to prevent pending actions.

### Transitions

S3 supports specific lifecycle transitions between storage classes as follows:
...
- Objects smaller than 128 KB are **not** transitioned by default.

  This applies to transitions to all storage classes.
  The default can be overridden with an [object size filter][lifecycle / general considerations for transitions].

- Some storage classes have a minimum storage duration.<br/>
  Objects **cannot** be _automatically_ transitioned before the end of the current storage class's minimum storage
  duration.

  A single rule cannot also chain transitions faster than all the minimums allow.<br/>
  E.g. transitioning to Glacier Instant Retrieval (90-day minimum) and then to Deep Archive requires at least 90 days
  between the two steps.

- Each object archived to Glacier Flexible Retrieval or Deep Archive carries 40 KB of data overhead (8 KB of metadata
  charged at Standard rates, plus 32 KB of index data charged at the destination Glacier rate). Archiving many small
  objects is disproportionately expensive.

See [General considerations for transitions][lifecycle / general considerations for transitions] for the full list.

### Expiration

**Permanently** removes objects or object versions after a specified period.

In buckets **with** versioning, expiring the current version of an object only adds a delete marker instead or pruning
the object, which makes that version _noncurrent_.

`NoncurrentVersionExpiration` **separately** controls when noncurrent versions are permanently deleted, and can retain a
specified number of recent noncurrent versions (up to 100).

One needs to use a dedicated lifecycle action to automatically clean up expired object delete markers (delete markers
with zero remaining noncurrent versions).

### Incomplete multipart uploads

The `AbortIncompleteMultipartUpload` action tells S3 to abort multipart uploads that are not completed within the
specified number of days, which deletes the associated parts.<br/>
This only affects **upload parts**. Complete objects remain intact.

> [!tip]
> Configure `AbortIncompleteMultipartUpload` on **all** buckets to prevent orphaned parts from accumulating storage
> costs.<br/>
> Object expiration rules do **not** remove incomplete multipart uploads. A dedicated action is required for this.
>
> `AbortIncompleteMultipartUpload` only supports prefix-based filters, and does **not** work on tags.

### Rule precedence

When multiple rules apply to the same object on the same day, S3 follows a defined precedence:
...
- [Expiring objects][lifecycle / expiring objects]
- [Configuring lifecycle rules to abort incomplete multipart uploads][lifecycle / abort incomplete multipart upload]
...
[lifecycle / abort incomplete multipart upload]: https://docs.aws.amazon.com/AmazonS3/latest/userguide/mpu-abort-incomplete-mpu-lifecycle-config.html
...
[lifecycle / expiring objects]: https://docs.aws.amazon.com/AmazonS3/latest/userguide/lifecycle-expire-general-considerations.html
