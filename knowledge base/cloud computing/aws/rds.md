...
RDS limits passwords for DB instances to printable ASCII characters, and **abhors `/`, `"`, spaces, and `@`**.<br/>
Their allowed length depends on the DB engine. Refer to [Settings for DB instances].

The initial database name set at instance creation **cannot** be changed through the API, but it _can_ be renamed on
server level using SQL commands (e.g., `ALTER DATABASE old_name RENAME TO new_name;`).<br/>
Renaming the DB that way will create a mismatch, where AWS continues reporting the old DB name but the connection string
and settings must use the new name.
...
1. Enable the required configuration parameters and `pg_transport` extension on the source and target RDS instances.<br/>
   ...
   - `pg_transport.work_mem` _can_ be tuned.<br/>
     Specifies the maximum memory to allocate to each worker. Defaults to `131072` (128 MB) or `262144` (256 MB)
