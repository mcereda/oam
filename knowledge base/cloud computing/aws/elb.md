...
# Priority 1 is the highest, 50000 is the lowest.
aws --profile 'rw-profile' elbv2 create-rule \
...
Should one do this, the LB will use the default certificate **only** when:
...
**Every** ALB requires to be assigned 2 or more subnets.<br/>
An ALB creates an ENI in each of those subnets. Each ENI always gets a **private** IP from the CIDR of the subnet they
are created in. For **public** (_Internet-facing_) ALBs, each ENI **also** gets a **public** IP from AWS' EC2 public
IPv4 address pool, which makes them directly reachable from the Internet.

When reached from **within** the VPC they reside in, traffic stays **inside** AWS' network (the traffic does **not**
exit and come back in).

Traffic from the **public** subnets goes directly to the **Internet** Gateway, which routes it back to the ALB
**without** processing it. The ALB sees the **public** IP of the client (not the IGW's).<br/>
The AWS account does **not** get charged for data out **nor** in.

```mermaid
---
title: Traffic flow from public subnets
---
flowchart LR
  c("Client") --> ig("IG")
  ig --Client's public IP--> alb("ALB")
  alb --> l("Listener") --> t("Target Group") --> s("Service")
```

Traffic from the **private** subnets goes to the **NAT** Gateway, which **processes** it and forwards it to the
**Internet** Gateway, which re-routes it to the ALB **without** processing it. The ALB sees the **public** IP of the
**NAT Gateway** (not the IGW's).<br/>
The AWS account **does** get charged for data the NAT gateway processes from private subnets.

```mermaid
---
title: Traffic flow from private subnets
---
flowchart LR
  c("Client") --> ng("NAT Gateway")
  ng --> ig("IG") --NAT Gateway's public IP--> alb("ALB")
  alb --> l("Listener") --> t("Target Group") --> s("Service")
```

Security group rules that allow the VPC's CIDR will **not** match the source of any of the above traffic flows, because
the CIDR is a private address and the ALB is presented a public IP.

**Private** (_internal_) ALBs are reachable from **any** subnet in the VPC (public or private). The VPC's `local` route
covers **all** VPC CIDR addresses in **every** route table. Hosts residing anywhere that has a route to the VPC CIDR can
reach internal ALBs just fine. This includes public subnets, peered VPCs, VPNs, and Direct Connect.

Traffic to a **private** ALB uses the VPC's `local` route. The destination is a **private** IP, so the traffic never
leaves the VPC routing and does **not** pass through NAT Gateways **nor** IGWs, meaning **no** data processing
applies.<br/>
The ALB sees the **private** IP of the **client**.

Cross-AZ transfer charges are the same either way.

ALBs' HTTP listeners require X.509 certificates (SSL/TLS server certificates).<br/>
