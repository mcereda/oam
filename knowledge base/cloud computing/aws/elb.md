...
An **internet-facing** ALB is reachable from **within** its VPC at no extra cost.<br/>
Traffic from the ALB's VPC, including the packets generated by VPN users through a subnet router in that same VPC, hits
the ALB via the Internet Gateway but stays **inside** the AWS's network. It _should not_ incur in charges for outgoing
data transfers. One _should_ pay the standard ALB hourly + LCU + cross-AZ transfers should the ALB node and target be
in different AZs, but that's the same total cost one would pay with an internal ALB anyway.
