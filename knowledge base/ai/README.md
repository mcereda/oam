...
Current **generative** AI systems sometimes generate **false** but **plausible-sounding** outputs and present them
**confidently**. This output is known as _hallucination_.

Hallucinations do **not** correspond to facts and can take the form of:

- **Novel fabrications**, when the output has **no** basis in training data **nor** is backed by reality.<br/>
  Examples include completely _inventing_ facts and describing non-existent objects (e.g. a source paper).
- **Distorted extrapolations**, when the system _recombines_ or _misrepresents_ patterns from its training data so that
  its output comes out wrong.<br/>
  Examples include objects blending in impossible ways in images generated by early vision model.

Generative models make _predictions_ based on **learned patterns**, rather than retrieving or verifying information.
This makes them _inherently_ prone to hallucination, especially where errors compound (like language models predicting
the likely next token).

Hallucinations are sometimes compared to _confabulation_, which occurs when people **unconsciously** fill memory gaps
with fabricated details they **genuinely** believe.<br/>
AI systems don't currently possess real memory but only learned statistical patterns. When applied metaphorically to AI,
the term _confabulation_ ends up being more an **analogy** than a precise description of the phenomenon.

Hallucinations pose serious risks specially in those high-stakes fields where users may trust AI-generated content
without verifying it, like medicine, law, and engineering.
